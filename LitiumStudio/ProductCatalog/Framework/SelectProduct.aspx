<%@ page language="C#" autoeventwireup="true" inherits="LitiumStudio_ProductCatalog_Framework_SelectProduct, Litum.Studio.Web" %>
...
<body>
	...
	<div style="width: 603px;" class="lsContentHeaderBackground">
		...
		<LS:GridHelperAjax runat="server" AssociatedControlID="c_productGrid" OnNeedData="productGrid_NeedData"
			ID="c_productGridHelper" DataKeyName="ID" />
		<Telerik:RadGrid AllowPaging="true" runat="server" ID="c_productGrid" Width="99%" OnItemCreated="grid_ItemCreated"
		...
		<div style="text-align: right; padding: 4px;">
			<asp:Button ID="ButtonCancel" CssClass="litInputButton" OnClientClick="RadClose();" runat="server" UseSubmitBehavior="false" />
			<asp:Button ID="ButtonAdd" CssClass="litInputButton" runat="server" OnClick="ButtonAdd_Click" />
		</div>
	</div>
	<Telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
		...
		<script type="text/javascript">
			...
			function SelectAllItems() {
				var tableView = $find("<%= c_productGrid.ClientID %>").get_masterTableView();
			...
			function HeaderShowing(sender, args) {
				$get($get('<%= c_hiddenSelectAllClientID.ClientID %>').value).checked = selectedItemsCount == $find("<%= c_productGrid.ClientID %>").get_masterTableView()._virtualItemCount;
			}
...
//					return;
				...
				$find("<%=c_productGrid.ClientID %>").get_masterTableView().page("First");
