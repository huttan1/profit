...
{
	public partial class ObjectPostAdmin : BasePage
	{
		private const string CURRENT_PAGE = "ObjectPostAllAdmin.aspx";
		...
		/// Default page size. 
		/// </summary>
		protected int m_defaultPageSize = -1;
		...
		protected short ObjectPostType
		{
			...
			set
			{
				Session["SelectedObjectPostType"] = value;
			}
		}
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				short selectedType = ObjectPostType;
				if (selectedType == short.MinValue)
					selectedType = (short)ObjectPostTypes.CMS_PAGE;

				ObjectPostAdministrationUtilities.InitDropDownObjectPostTypesAllPosts((ObjectPostTypes)selectedType, m_dropDownListObjectPostTypes, WebSiteID);
		...
		protected void ToolbarItemDelete_Click(object sender, EventArgs e)
		{
			...
			if (c_list.SelectedItemIDs.Count > 0)
			{
				...
				{
					isDeleted = false;
					FoundationContext.Solution.Log.CreateLogEntry(GetType().FullName + "ToolbarItemDelete_Click", exc, LogLevels.WARN);
				}
				if (!isDeleted)
				{
					// Not deleted show error message
					c_labelMajorError.Text = SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_DELETE_ERROR, FoundationContext.Language.ID, true);
		...
		protected void List_Command(object sender, CommandEventArgs e)
		{
			switch (e.CommandName.ToLower())
			{
				...
				case StringConstants.COMMAND_NAME_GOTO:
					int pageRequested = c_list.PageSelected + Int32.Parse(e.CommandArgument.ToString());
		...
		{
			m_toolbarItemEdit.Text = SystemStrings.GetValue(Litium.Foundation.GUI.Constants.SystemStringConstants.TOOLBAR_EDIT, FoundationContext.Language.ID, true);
			m_toolbarItemEdit.Enabled = false;

			m_toolbarItemDelete.Text = SystemStrings.GetValue(Litium.Foundation.GUI.Constants.SystemStringConstants.TOOLBAR_DELETE, FoundationContext.Language.ID, true);
			m_toolbarItemDelete.ConfirmationText = SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_TOOLBAR_DELETE_CONFIRM_TEXT, FoundationContext.Language.ID, true);
		...
		private void InitLabels()
		{
			...
			if (m_listState.PageSize == c_list.SHOW_ALL)
			{
				c_labelShowAll.Text = SystemStrings.GetValue(SystemStringConstants.LABEL_SHOW_PAGES, FoundationContext.Language.ID, true);
			}
			else
			{
				c_labelShowAll.Text = SystemStrings.GetValue(SystemStringConstants.LABEL_SHOW_ALL, FoundationContext.Language.ID, true);
			}
		}
		...
		private void InitList()
		{
			...
			c_list.AddColumn("c_listColumn1", SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_LABEL_DATE, FoundationContext.Language.ID, true), StringConstants.KEY_COLUMN1, "c_imageDescListColumn1", true);
			...
			c_list.AddColumn("c_listColumn2", SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_LABEL_NAME, FoundationContext.Language.ID, true), StringConstants.KEY_COLUMN2, "c_imageDescListColumn2", true);
			c_list.AddColumn("c_listColumn3", SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_LABEL_EMAIL, FoundationContext.Language.ID, true), StringConstants.KEY_COLUMN3, "c_imageDescListColumn3", true);
			c_list.AddColumn("c_listColumn4", SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_LABEL_TEXT, FoundationContext.Language.ID, true), StringConstants.KEY_COLUMN4, "c_imageDescListColumn4", true);
			c_list.AddColumn("c_listColumn5", SystemStrings.GetValue(ObjectPostConstants.SYSTEM_STRING_LABEL_LINK, FoundationContext.Language.ID, true), StringConstants.KEY_COLUMN5, "c_imageDescListColumn5", true);
